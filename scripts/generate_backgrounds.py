...
def main():
    ...
    # Merge in real photographs, if the importers that produce them have been
    # run. Those files aren't generated by this script and are left untouched
    ...
    for manifest_name, importer in (
        ...
        for entry in photos:
            if not os.path.exists(os.path.join(ROOT, entry["filename"])):
                print(f"WARNING: {entry['filename']} listed in {manifest_name} but "
                      f"missing on disk -- run {importer}. Skipping.")
                continue
